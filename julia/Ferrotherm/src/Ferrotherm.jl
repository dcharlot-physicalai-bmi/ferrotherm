...
search. Supported today for Pegasus (`K_{12(m-1)}`, chains <= `m+1` -- `K_180` on the Advantage's P16)
...
On BOTH Pegasus and Zephyr this IS the maximum `busclique` reaches on a perfect fabric, at the same
chain length. Either way it beats this crate's own search decisively and is instant. Errors when the topology has no known construction; `embed!` is the fallback.
"""

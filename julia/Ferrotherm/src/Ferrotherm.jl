...
search. Supported today for Pegasus (`K_{12(m-1)-4}`, chains <= `m+1` -- `K_176` on the Advantage's P16)
...
Pegasus it is within 3% (the last four chains need per-pair arm trimming, which this crate does not
perform). Either way it beats this crate's own search decisively and is instant. Errors when the topology has no known construction; `embed!` is the fallback.
"""

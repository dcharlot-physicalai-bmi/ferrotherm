...
impl FixedPdit {
    ...
    /// Emit an AXI4-Lite control shell around this fabric: the same register map as
    /// [`crate::hdl::FixedFabric::emit_axi_shell`], so the host code that drives a p-bit fabric on
    /// a KV260 or an F2 drives a p-dit fabric unchanged, with two differences a radix forces:
    ///
    ...
    /// | `0x10` | RO | sites in state zero -- the Potts order parameter's one-word stand-in |
    /// | `0x20 + 4k` | RO | state words, `ceil(n x bits / 32)` of them, site `i` at bits `i x bits ..` |
    ///
    /// A sweep is one clock per colour class, so the counter advances on the clock that updates
    /// the last class -- and the fabric then stops with exactly the target's sweeps done. The
    /// state-zero count is pipelined per 32-site word as the p-bit shell's popcount is, for the
    /// same critical-path reason.
    #[must_use]
    pub fn emit_axi_shell(&self, module: &str, core: &str, clock: &str) -> String {
        ...
        let classes = self.classes.len();
        let top = n * sb - 1;
        let words = (n * sb).div_ceil(32);
        // A sweep ends on the clock that updates the last colour class: count on that clock, so
        // the fabric stops with the target's sweeps done and not half of one more.
        let last_class = format!("(phase == {pb}'d{})", classes - 1);
        let mut state_words = String::new();
        for k in 0..words {
            let lo = k * 32;
            let hi = ((k + 1) * 32).min(n * sb) - 1;
            let width = hi - lo + 1;
            if width == 32 {
                state_words.push_str(&format!(
                    "          8'h{:02X}: rdata_r <= state[{hi}:{lo}];\n",
                    0x20 + 4 * k
                ));
            } else {
                state_words.push_str(&format!(
                    "          8'h{:02X}: rdata_r <= {{{}'d0, state[{hi}:{lo}]}};\n",
                    0x20 + 4 * k,
                    32 - width
                ));
            }
        ...
        format!(
            r#"// generated by ferrotherm::pdit -- AXI4-Lite control shell for a p-dit fabric
// {n} p-dits of radix {q}, {sb} bits per site, {classes} colour classes per sweep
module {module} (
    input  wire        {clock},
    input  wire        rst_n,
  ...
  // A sweep is {classes} clock(s); it ends on the clock that updates the last class.
  always @(posedge {clock}) begin
    if (rst) done_sweeps <= 32'd0;
    else if (run & ~reached) begin
      if ({last_class}) done_sweeps <= done_sweeps + 32'd1;
    end
  end

  // Sites in state zero, pipelined per 32-site word.
  localparam NS = {n};
  localparam NW = (NS + 31) / 32;
  wire [NW*32-1:0] zbits;
  genvar gi;
  generate for (gi = 0; gi < NW*32; gi = gi + 1) begin : zpad
    if (gi < NS) begin : real_site
      assign zbits[gi] = (state[gi*{sb} +: {sb}] == {sb}'d0);
    end else begin : zero_site
      assign zbits[gi] = 1'b0;
    end
  end endgenerate
  ...
  integer wi, wj;
  reg [31:0] pacc;
  always @(posedge {clock}) for (wi = 0; wi < NW; wi = wi + 1) cpop[wi] <= pc32(zbits[wi*32 +: 32]);
  always @(posedge {clock}) begin
    pacc = 32'd0;
  ...
  // AXI4-Lite slave: one outstanding transaction, the two write channels handshaken separately.
  ...
  always @(posedge {clock}) begin
    if (!rst_n) begin
      ...
      if (arready_r) begin
        rvalid_r <= 1'b1;
        case (araddr_r[7:0])
          8'h00: rdata_r <= {{31'd0, run}};
          8'h04: rdata_r <= {{30'd0, reached, run}};
          8'h08: rdata_r <= target;
          8'h0C: rdata_r <= done_sweeps;
          8'h10: rdata_r <= zeros;
{state_words}          default: rdata_r <= 32'd0;
        endcase
...
mod tests {
    ...
    /// THE SHELL GATE for the p-dit: the AXI4-Lite wrapper is driven as a host drives it, in
    /// simulation, on a two-class ring and a three-class five-state model; it must start the
    /// fabric, count sweeps, stop at the target, and read back the state words and the
    /// state-zero count the emulator reaches after exactly that many sweeps.
    #[test]
    fn the_p_dit_axi_shell_runs_the_fabric_and_reads_back_what_the_emulator_reaches() {
        ...
        let five = b.build();
        let sweeps = 25usize;
        for (name, m, beta) in [("ring6", &ring6, 0.7), ("five", &five, 0.9)] {
            let mut fab = FixedPdit::new(m, beta, 0x5EED, 10).expect("Potts");
            let core = fab.emit_verilog("pdit");
            let shell = fab.emit_axi_shell("pd_axi", "pdit", "clk");
            fab.reset();
            for _ in 0..sweeps {
                fab.sweep();
            }
            let sb = fab.state_bits();
            let total = fab.n * sb;
            let words = total.div_ceil(32);
            let mut want = vec![0u32; words];
            for (i, &v) in fab.s.iter().enumerate() {
                for bit in 0..sb {
                    if (v >> bit) & 1 == 1 {
                        let pos = i * sb + bit;
                        want[pos / 32] |= 1 << (pos % 32);
                    }
                }
            }
            let want_zeros = fab.s.iter().filter(|&&v| v == 0).count() as u32;
            let mut reads = String::new();
            for (k, w) in want.iter().enumerate() {
                reads.push_str(&format!(
                    "    rd(32'h{:02X}, got); if (got !== 32'd{w}) begin errors = errors + 1; $display(\"WORD {k}: got %h want %h\", got, 32'd{w}); end\n",
                    0x20 + 4 * k
                ));
            }
            let tb = format!(
...
module tb;
  ...
  pd_axi dut(.clk(clk), .rst_n(rst_n),
  ...
  initial begin
    ...
    wr(32'h00, 32'd1);
    guard = 0; got = 0;
    while (!got[1] && guard < 2000) begin rd(32'h04, got); guard = guard + 1; end
    if (!got[1]) begin errors = errors + 1; $display("TARGET NEVER REACHED"); end
    rd(32'h0C, got); if (got !== 32'd{sweeps}) begin errors = errors + 1; $display("SWEEPS: got %0d", got); end
    repeat (3) @(posedge clk);
    rd(32'h10, got); if (got !== 32'd{want_zeros}) begin errors = errors + 1; $display("ZEROS: got %0d want {want_zeros}", got); end
{reads}    if (errors == 0) $display("FERROTHERM_PASS"); else $display("FERROTHERM_FAIL %0d", errors);
    $finish;
  end
endmodule
"#
            ...
            let dir = std::env::temp_dir()
                .join(format!("ferrotherm_pdit_axi_{name}_{}", std::process::id()));
            std::fs::create_dir_all(&dir).unwrap();
            std::fs::write(dir.join("pdit.v"), core).unwrap();
            std::fs::write(dir.join("shell.v"), shell).unwrap();
            std::fs::write(dir.join("tb.v"), tb).unwrap();
            let out = std::process::Command::new("iverilog")
                .current_dir(&dir)
                .args(["-g2012", "-o", "sim", "pdit.v", "shell.v", "tb.v"])
                .output()
                .unwrap();
            assert!(
                out.status.success(),
